...
class MCPServerTemplate(TaskTemplate):
    ...
    def generate_tasks(self, context: Dict[str, Any]) -> List[Dict[str, Any]]:
        ...
        tasks.append({
            ...
            'dependencies': [1],
            'files_to_create': [
                f'{server_name}/src/{server_name.replace("-", "_")}_server.py'
            ],
            'type': 'mcp_server'
        })
        task_id += 1
        ...
        if include_docs:
            tasks.append({
                ...
                'description': f'Write README with features, usage, and examples',
                'estimated_loc': 150,
                'dependencies': [2],
                'files_to_create': [
...
class DatabaseTemplate(TaskTemplate):
    ...
    def __init__(self):
        super().__init__(
            name="database",
            description="Database schema, migrations, and data access layer"
        )
...
class TestingInfrastructureTemplate(TaskTemplate):
    ...
    def __init__(self):
        super().__init__(
            name="testing",
            description="Testing framework, fixtures, and CI integration"
        )
