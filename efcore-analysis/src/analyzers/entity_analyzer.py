...
class EntityAnalyzer:
    ...
    async def analyze(
        self, file_path: Path, entity_name: str | None = None
    ) -> list[dict[str, Any]]:
        ...
        entities = []
        class_matches = re.finditer(r"public\s+class\s+(\w+)(?:\s*:\s*([^{]+))?\s*{", content)
    ...
    async def validate(self, file_path: Path) -> dict[str, Any]:
        ...
        return {"total_entities": len(entities), "total_issues": len(issues), "issues": issues}
    ...
    def _extract_properties(self, content: str, class_name: str) -> list[dict[str, Any]]:
        ...
        class_match = re.search(
            rf"class\s+{class_name}\s*(?::\s*[^{{]+)?\s*{{([^}}]+)}}", content, re.DOTALL
        )
        if not class_match:
            return properties
    ...
    def _extract_navigation_properties(self, content: str, class_name: str) -> list[dict[str, Any]]:
        ...
        for match in re.finditer(ref_pattern, class_body):
            ...
            if (
                prop_type
                not in ["string", "int", "long", "decimal", "DateTime", "bool", "double", "float"]
                and prop_type[0].isupper()
                and not prop_name.endswith("Id")
            ):
                nav_props.append(
                    {
                        "name": prop_name,
    ...
    def _find_primary_key(self, content: str, class_name: str) -> str | None:
