...
async def calculate_complexity(
    ...
    if isinstance(complexities, dict) and complexities:
        avg_complexity = sum(complexities.values()) / len(complexities) if isinstance(list(complexities.values())[0], int) else 1

        return {
            "file_complexity": avg_complexity,
            "function_complexities": complexities,
            "high_complexity_functions": [
                name for name, score in complexities.items()
                if (isinstance(score, int) and score > 10) or
                   (isinstance(score, dict) and score.get("cyclomatic", 0) > 10)
            ]
        }
